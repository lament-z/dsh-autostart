...
- **配置卡片**（设置 → 插件 → 插件配置 → 「DSH 自启动 / 重启」）：可视化编辑以下设置，改动即时写入 `settings.yaml`：
  - `autostartEnabled` / `autostartProfile` — 开机自启动开关与 profile（本插件新增）。
...
{
  "dependencies": { "dsh-autostart": "..." },
  "dsh": { "profile": { "bundles": ["...", "dsh-autostart"] } }
}
```
...
```sh
# 从 npm 安装
dsh plugin --profile web add npm:@lament_z/dsh-autostart
...
重启 DSH（`/restart` 或 `restart_harness`），刷新页面后即可看到卡片；之后通过卡片重启时会自动等待并恢复页面。
...
git init            # 仓库已初始化则跳过
git add -A
git commit -m "feat: dsh-autostart — 开机自启动 + 重启/看门狗"
git remote add origin git@github.com:lament-z/dsh-autostart.git
git push -u origin main

# 2) 打 tag 触发 npm 发布（需在仓库 Secrets 配置 NPM_TOKEN）
npm version patch   # 同步 package.json 版本
git tag v$(node -p "require('./package.json').version")
git push origin --tags
```

其他 deepseek-harness 实例可通过 `dsh plugin --profile web add npm:@lament_z/dsh-autostart` 或 `github:lament-z/dsh-autostart` 拉取本插件。
