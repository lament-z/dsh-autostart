...
function autostartStateFilePath() {
...
function readAutostartState() {
    const fallback = { installed: false, method: null, command: null, error: null, at: '' };
    try {
...
function buildLaunchdPlist(label, args, cwd) {
...
function buildSystemdUnit(command) {
    return `[Unit]
...
function installAutostart(dynamic) {
    ...
    try {
        if (platform === 'darwin') {
            ...
            if (uid)
                runSync('launchctl', ['bootstrap', `gui/${uid}`, plistPath]);
...
function uninstallAutostart() {
    ...
    try {
        if (platform === 'darwin') {
            ...
            try {
                fs.unlinkSync(path.join(os.homedir(), '.config', 'systemd', 'user', 'dsh-autostart.service'));
            }
            catch { /* already gone */ }
        }
    }
...
function syncAutostart(dynamic) {
    try {
        if (dynamic().autostartEnabled)
            installAutostart(dynamic);
        else
            uninstallAutostart();
    }
    catch (error) {
